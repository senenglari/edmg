...
<button id="openNewTab">Buka di tab baru</button>
<a href="?pdf_id=5&cek=1">tes</a>

<?php 
$cek = isset($_GET['cek']) ? intval($_GET['cek']) : 0;

?>

<script>
  document.getElementById("openNewTab").addEventListener("click", function () {
    // URL yang sama persis dengan halaman sekarang (termasuk query ?pdf_id=...)
    ...
    // fallback: kalau browser menolak close, lempar tab lama ke halaman aman
    setTimeout(function () {
      if (!window.closed) {
        window.location.replace("/pdf-stamp/index.php?pdf_id=5&x=2"); // ganti kalau mau ke halaman lain
      }
    }, 300);
  });
</script>
