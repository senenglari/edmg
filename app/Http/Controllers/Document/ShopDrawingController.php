...
class ShopDrawingController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            $data["form_act"]         = "/shopdrawing/index";
            ...
            # ---------------
            $data["query"]         = $this->qDocument->getIFIContructionCollections();
            ...
            if (isset($request->module_id)) {
                ...
                # ---------------
                return redirect("/shopdrawing/index");
            }
            # ---------------
            ...
            $data["buttons"][]     = form_action_button(array("name" => "button_clear", "label" => "&nbsp;&nbsp;Clear&nbsp;&nbsp;", "url" => "/shopdrawing/unfilter"));
            # ---------------
    ...
    public function detail($id) {
        try {
            $data["title"]         = "Detail";
            $data["parent"]        = ucwords(strtolower($this->PROT_Parent));
            $data["form_act"]      = "/shopdrawing/update_status";
            # ---------------
            $id                    = decodedData($id);
            ...
            $data["fields"][]      = form_file(array("name"=>"document", "label"=>"Document IFC", "readonly"=>"readonly", "value"=>$file));
            ...
            # ---------------
            $data["buttons"][]     = form_button_update(array("name"=>"button_save", "label"=>"&nbsp;&nbsp;Cancel&nbsp;&nbsp;"));
            # ---------------
            $response   = $this->qDocument->updateStatus($id);
    ...
    public function update_status(Request $request) {
        // dd($request);
        try {
            $response   = $this->qDocument->updateStatusCancle($request);
            ...
            $this->logModel->createError($e->getMessage(), "PAGE UPDATE STATUS SHOP DRAWING", "");
            # ---------------
            return view("error.405");
        }
    }
