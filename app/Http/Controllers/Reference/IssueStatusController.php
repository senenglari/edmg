...
class IssueStatusController extends Controller
{
    ...
    public function __construct(Request $request) {
        ...
        $this->qIssueStatus       = new IssueStatusModel;
    ...
    public function index(Request $request)
    {
        try {
            ...
            $data["form_act"]         = "/issue_status/index";
                                                          ...
                                                          ,"item-format"=>"flag"
                                                            ,"item-class"=>""
                                                              ,"width"=>"15%"
                                                                ,"add-style"=>""));
            # ---------------
            $data["query"]         = $this->qIssueStatus->getCollections();
            ...
            if(isset($request->module_id)) {
                $name              = ($request->name != "") ? session(["SES_SEARCH_ISSUE_STATUS_NAME" => $request->name]) : $request->session()->forget("SES_SEARCH_ISSUE_STATUS_NAME");
                # ---------------
                return redirect("/issue_status/index");
            }
            # ---------------
            if($request->session()->has("SES_SEARCH_ISSUE_STATUS_NAME")) {
            ...
            $data["fields"][]      = form_search_text(array("name"=>"name", "label"=>"Name", "value"=>($request->session()->has("SES_SEARCH_ISSUE_STATUS_NAME")) ? $request->session()->get("SES_SEARCH_ISSUE_STATUS_NAME") : ""));
            # ---------------
            $data["buttons"][]     = form_button_submit(array("name"=>"button_search", "label"=>"&nbsp;&nbsp;Search&nbsp;&nbsp;"));
            $data["buttons"][]     = form_action_button(array("name"=>"button_clear", "label"=>"&nbsp;&nbsp;Clear&nbsp;&nbsp;", "url"=>"/issue_status/unfilter"));
            # ---------------
            return view("default.list", $data);
        } catch (\Exception $e) {
            throw $e;
            $this->logModel->createError($e->getMessage(), "PAGE ISSUE STATUS", "");
            # ---------------
    ...
    public function unfilter() {
        session()->forget("SES_SEARCH_ISSUE_STATUS_NAME");
        # ---------------
        return redirect("/issue_status/index");
    }
    ...
    public function add() {
        try {
            $data["title"]         = "Add Issue Status";
            $data["parent"]        = ucwords(strtolower($this->PROT_Parent));
            $data["form_act"]      = "/issue_status/save";
            /* ----------
             Model
            ----------------------- */
    ...
    public function save(Request $request) {
        try {
            $response   = $this->qIssueStatus->createData($request);
    ...
    public function edit($id) {
        try {
            $data["title"]        = "Edit Issue Status";
            $data["parent"]       = ucwords(strtolower($this->PROT_Parent));
            $data["form_act"]     = "/issue_status/update";
            # ---------------
            $id                   = decodedData($id);
            /* ----------
             Model
            ...
            /* ----------
             Source
            ----------------------- */
            $qIssueStatus          = IssueStatusModel::find($id);
            ...
            $data["fields"][]      = form_text(array("name"=>"name", "label"=>"Name", "mandatory"=>"yes", "readonly"=>"readonly", "value"=>$qIssueStatus->name));
            $data["fields"][]      = form_radio(array("name"=>"status", "label"=>"Status", "mandatory"=>"yes", "source"=>$qStatus, "value"=>$qIssueStatus->status));
            # ---------------
    ...
    public function update(Request $request) {
        try {
            $response   = $this->qIssueStatus->updateData($request);
            ...
            $this->logModel->createError($e->getMessage(), "PAGE UPDATE ISSUE STATUS", "");
            # ---------------
            return view("error.405");
        }
    }
