...
use App\Model\Reference\AreaModel;
...
class AreaController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
                                                          ...
                                                          ,"item-format"=>"flag"
                                                            ,"item-class"=>""
                                                              ,"width"=>"15%"
                                                                ,"add-style"=>""));
            # ---------------
            $data["query"]         = $this->qArea->getCollections();
            ...
            if(isset($request->module_id)) {
                $name              = ($request->name != "") ? session(["SES_SEARCH_AREA_NAME" => $request->name]) : $request->session()->forget("SES_SEARCH_AREA_NAME");
                # ---------------
                return redirect("/area/index");
            }
            # ---------------
            if($request->session()->has("SES_SEARCH_AREA_NAME")) {
            ...
            $data["fields"][]      = form_search_text(array("name"=>"name", "label"=>"Name", "value"=>($request->session()->has("SES_SEARCH_AREA_NAME")) ? $request->session()->get("SES_SEARCH_AREA_NAME") : ""));
            # ---------------
            $data["buttons"][]     = form_button_submit(array("name"=>"button_search", "label"=>"&nbsp;&nbsp;Search&nbsp;&nbsp;"));
            $data["buttons"][]     = form_action_button(array("name"=>"button_clear", "label"=>"&nbsp;&nbsp;Clear&nbsp;&nbsp;", "url"=>"/area/unfilter"));
            # ---------------
    ...
    public function unfilter() {
        session()->forget("SES_SEARCH_AREA_NAME");
        # ---------------
        return redirect("/area/index");
    }
    ...
    public function save(Request $request) {
        try {
            $response   = $this->qArea->createData($request);
    ...
    public function edit($id) {
        try {
            ...
            $data["fields"][]      = form_text(array("name"=>"code", "label"=>"Code", "mandatory"=>"yes", "readonly"=>"readonly", "value"=>$qArea->code));
            $data["fields"][]      = form_text(array("name"=>"name", "label"=>"Name", "mandatory"=>"yes", "readonly"=>"readonly", "value"=>$qArea->name));
            $data["fields"][]      = form_radio(array("name"=>"status", "label"=>"Status", "mandatory"=>"yes", "source"=>$qStatus, "value"=>$qArea->status));
            # ---------------
    ...
    public function update(Request $request) {
        try {
            $response   = $this->qArea->updateData($request);
            ...
            $this->logModel->createError($e->getMessage(), "PAGE UPDATE AREA", "");
            # ---------------
            return view("error.405");
        }
    }
