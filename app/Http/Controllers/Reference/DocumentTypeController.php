...
use App\Model\Reference\DocumentTypeModel;
...
class DocumentTypeController extends Controller
{
    ...
    public function __construct(Request $request) {
        ...
        $this->qDocumentType      = new DocumentTypeModel;
    ...
    public function index(Request $request)
    {
        try {
            ...
            $data["form_act"]         = "/document_type/index";
                                                          ...
                                                          ,"item-format"=>"flag"
                                                            ,"item-class"=>""
                                                              ,"width"=>"15%"
                                                                ,"add-style"=>""));
            # ---------------
            $data["query"]         = $this->qDocumentType->getCollections();
            ...
            if(isset($request->module_id)) {
                $name              = ($request->name != "") ? session(["SES_SEARCH_DOCUMENT_TYPE_NAME" => $request->name]) : $request->session()->forget("SES_SEARCH_DOCUMENT_TYPE_NAME");
                # ---------------
                return redirect("/document_type/index");
            }
            # ---------------
            if($request->session()->has("SES_SEARCH_DOCUMENT_TYPE_NAME")) {
            ...
            # ---------------
            $data["fields"][]      = form_hidden(array("name"=>"module_id", "label"=>"Module ID", "value"=>"DOCUMENT_TYPE"));
            $data["fields"][]      = form_search_text(array("name"=>"name", "label"=>"Name", "value"=>($request->session()->has("SES_SEARCH_DOCUMENT_TYPE_NAME")) ? $request->session()->get("SES_SEARCH_DOCUMENT_TYPE_NAME") : ""));
            # ---------------
            $data["buttons"][]     = form_button_submit(array("name"=>"button_search", "label"=>"&nbsp;&nbsp;Search&nbsp;&nbsp;"));
            $data["buttons"][]     = form_action_button(array("name"=>"button_clear", "label"=>"&nbsp;&nbsp;Clear&nbsp;&nbsp;", "url"=>"/document_type/unfilter"));
            # ---------------
            return view("default.list", $data);
        } catch (\Exception $e) {
            throw $e;
            $this->logModel->createError($e->getMessage(), "PAGE DOCUMENT TYPE", "");
            # ---------------
    ...
    public function unfilter() {
        session()->forget("SES_SEARCH_DOCUMENT_TYPE_NAME");
        # ---------------
        return redirect("/document_type/index");
    }
    ...
    public function add() {
        try {
            ...
            $data["parent"]        = ucwords(strtolower($this->PROT_Parent));
            $data["form_act"]      = "/document_type/save";
            /* ----------
             Model
            ----------------------- */
    ...
    public function save(Request $request) {
        try {
            $response   = $this->qDocumentType->createData($request);
    ...
    public function edit($id) {
        try {
            ...
            $data["parent"]       = ucwords(strtolower($this->PROT_Parent));
            $data["form_act"]     = "/document_type/update";
            # ---------------
            $id                   = decodedData($id);
            /* ----------
             Model
            ...
            /* ----------
             Source
            ----------------------- */
            $qDocumentType         = DocumentTypeModel::find($id);
            ...
            $data["fields"][]      = form_text(array("name"=>"code", "label"=>"Code", "mandatory"=>"yes", "readonly"=>"readonly", "value"=>$qDocumentType->code));
            $data["fields"][]      = form_text(array("name"=>"name", "label"=>"Name", "mandatory"=>"yes", "readonly"=>"readonly", "value"=>$qDocumentType->name));
            $data["fields"][]      = form_radio(array("name"=>"status", "label"=>"Status", "mandatory"=>"yes", "source"=>$qStatus, "value"=>$qDocumentType->status));
            # ---------------
    ...
    public function update(Request $request) {
        try {
            $response   = $this->qDocumentType->updateData($request);
            ...
            $this->logModel->createError($e->getMessage(), "PAGE UPDATE DOCUMENT TYPE", "");
            # ---------------
            return view("error.405");
        }
    }
