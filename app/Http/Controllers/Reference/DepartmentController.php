...
use App\Model\Reference\DepartmentModel;
...
class DepartmentController extends Controller
{
    ...
    public function __construct(Request $request) {
        ...
        $this->qDepartment        = new DepartmentModel;
    ...
    public function index(Request $request)
    {
        try {
            ...
            $data["form_act"]         = "/department/index";
                                                          ...
                                                          ,"item-format"=>"flag"
                                                            ,"item-class"=>""
                                                              ,"width"=>"15%"
                                                                ,"add-style"=>""));
            # ---------------
            $data["query"]         = $this->qDepartment->getCollections();
            ...
            if(isset($request->module_id)) {
                $name              = ($request->name != "") ? session(["SES_SEARCH_DEPARTMENT_NAME" => $request->name]) : $request->session()->forget("SES_SEARCH_DEPARTMENT_NAME");
                # ---------------
                return redirect("/department/index");
            }
            # ---------------
            if($request->session()->has("SES_SEARCH_DEPARTMENT_NAME")) {
            ...
            $data["fields"][]      = form_search_text(array("name"=>"name", "label"=>"Name", "value"=>($request->session()->has("SES_SEARCH_DEPARTMENT_NAME")) ? $request->session()->get("SES_SEARCH_DEPARTMENT_NAME") : ""));
            # ---------------
            $data["buttons"][]     = form_button_submit(array("name"=>"button_search", "label"=>"&nbsp;&nbsp;Search&nbsp;&nbsp;"));
            $data["buttons"][]     = form_action_button(array("name"=>"button_clear", "label"=>"&nbsp;&nbsp;Clear&nbsp;&nbsp;", "url"=>"/department/unfilter"));
            # ---------------
    ...
    public function save(Request $request) {
        try {
            $response   = $this->qDepartment->createData($request);
    ...
    public function edit($id) {
        try {
            ...
            $data["parent"]       = ucwords(strtolower($this->PROT_Parent));
            $data["form_act"]     = "/department/update";
            # ---------------
            $id                   = decodedData($id);
            /* ----------
             Model
            ...
            /* ----------
             Source
            ----------------------- */
            $qDepartment           = DepartmentModel::find($id);
            ...
            $data["fields"][]      = form_text(array("name"=>"name", "label"=>"Name", "mandatory"=>"yes", "readonly"=>"readonly", "value"=>$qDepartment->name));
            $data["fields"][]      = form_radio(array("name"=>"status", "label"=>"Status", "mandatory"=>"yes", "source"=>$qStatus, "value"=>$qDepartment->status));
            # ---------------
    ...
    public function update(Request $request) {
        try {
            $response   = $this->qDepartment->updateData($request);
            ...
            $this->logModel->createError($e->getMessage(), "PAGE UPDATE DEPARTMENT", "");
            # ---------------
            return view("error.405");
        }
    }
