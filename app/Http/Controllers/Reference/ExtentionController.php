...
use App\Model\Reference\ExtentionModel;
...
class ExtentionController extends Controller
{
    ...
    public function __construct(Request $request) {
        ...
        $this->qExtention         = new ExtentionModel;
    ...
    public function index(Request $request)
    {
        try {
                                                          ...
                                                          ,"item-format"=>"flag"
                                                            ,"item-class"=>""
                                                              ,"width"=>"15%"
                                                                ,"add-style"=>""));
            # ---------------
            $data["query"]         = $this->qExtention->getCollections();
            ...
            if(isset($request->module_id)) {
                $name              = ($request->name != "") ? session(["SES_SEARCH_EXTENTION_NAME" => $request->name]) : $request->session()->forget("SES_SEARCH_EXTENTION_NAME");
                # ---------------
                return redirect("/extention/index");
            }
            # ---------------
            if($request->session()->has("SES_SEARCH_EXTENTION_NAME")) {
            ...
            # ---------------
            $data["fields"][]      = form_hidden(array("name"=>"module_id", "label"=>"Module ID", "value"=>"EXTENTION"));
            $data["fields"][]      = form_search_text(array("name"=>"name", "label"=>"Name", "value"=>($request->session()->has("SES_SEARCH_EXTENTION_NAME")) ? $request->session()->get("SES_SEARCH_EXTENTION_NAME") : ""));
            # ---------------
            $data["buttons"][]     = form_button_submit(array("name"=>"button_search", "label"=>"&nbsp;&nbsp;Search&nbsp;&nbsp;"));
            $data["buttons"][]     = form_action_button(array("name"=>"button_clear", "label"=>"&nbsp;&nbsp;Clear&nbsp;&nbsp;", "url"=>"/extention/unfilter"));
            # ---------------
            return view("default.list", $data);
        } catch (\Exception $e) {
            throw $e;
            $this->logModel->createError($e->getMessage(), "PAGE EXTENTION", "");
            # ---------------
    ...
    public function unfilter() {
        session()->forget("SES_SEARCH_EXTENTION_NAME");
        # ---------------
        return redirect("/extention/index");
    }
    ...
    public function add() {
        try {
            $data["title"]         = "Add Extention";
            $data["parent"]        = ucwords(strtolower($this->PROT_Parent));
            $data["form_act"]      = "/extention/save";
            /* ----------
             Model
            ----------------------- */
    ...
    public function save(Request $request) {
        try {
            $response   = $this->qExtention->createData($request);
    ...
    public function edit($id) {
        try {
            $data["title"]        = "Edit Extention";
            $data["parent"]       = ucwords(strtolower($this->PROT_Parent));
            $data["form_act"]     = "/extention/update";
            # ---------------
            $id                   = decodedData($id);
            /* ----------
             Model
            ...
            /* ----------
             Source
            ----------------------- */
            $qExtention            = ExtentionModel::find($id);
            ...
            $data["fields"][]      = form_text(array("name"=>"name", "label"=>"Name", "mandatory"=>"yes", "readonly"=>"readonly", "value"=>$qExtention->name));
            $data["fields"][]      = form_radio(array("name"=>"status", "label"=>"Status", "mandatory"=>"yes", "source"=>$qStatus, "value"=>$qExtention->status));
            # ---------------
    ...
    public function update(Request $request) {
        try {
            $response   = $this->qExtention->updateData($request);
            ...
            $this->logModel->createError($e->getMessage(), "PAGE UPDATE EXTENTION", "");
            # ---------------
            return view("error.405");
        }
    }
