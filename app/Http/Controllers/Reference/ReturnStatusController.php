...
use App\Model\Reference\ReturnStatusModel;
...
class ReturnStatusController extends Controller
{
    ...
    public function __construct(Request $request) {
        ...
        $this->qReturnStatus      = new ReturnStatusModel;
    ...
    public function index(Request $request)
    {
        try {
            ...
            $data["form_act"]         = "/return_status/index";
                                                          ...
                                                          ,"item-format"=>"flag"
                                                            ,"item-class"=>""
                                                              ,"width"=>"15%"
                                                                ,"add-style"=>""));
            # ---------------
            $data["query"]         = $this->qReturnStatus->getCollections();
            ...
            if(isset($request->module_id)) {
                $name              = ($request->name != "") ? session(["SES_SEARCH_RETURN_STATUS_NAME" => $request->name]) : $request->session()->forget("SES_SEARCH_RETURN_STATUS_NAME");
                # ---------------
                return redirect("/return_status/index");
            }
            # ---------------
            if($request->session()->has("SES_SEARCH_RETURN_STATUS_NAME")) {
            ...
            $data["fields"][]      = form_search_text(array("name"=>"name", "label"=>"Name", "value"=>($request->session()->has("SES_SEARCH_RETURN_STATUS_NAME")) ? $request->session()->get("SES_SEARCH_RETURN_STATUS_NAME") : ""));
            # ---------------
            $data["buttons"][]     = form_button_submit(array("name"=>"button_search", "label"=>"&nbsp;&nbsp;Search&nbsp;&nbsp;"));
            $data["buttons"][]     = form_action_button(array("name"=>"button_clear", "label"=>"&nbsp;&nbsp;Clear&nbsp;&nbsp;", "url"=>"/return_status/unfilter"));
            # ---------------
            return view("default.list", $data);
        } catch (\Exception $e) {
            throw $e;
            $this->logModel->createError($e->getMessage(), "PAGE RETURN CODE", "");
            # ---------------
    ...
    public function unfilter() {
        session()->forget("SES_SEARCH_RETURN_STATUS_NAME");
        # ---------------
        return redirect("/return_status/index");
    }
    ...
    public function add() {
        try {
            $data["title"]         = "Add Return Code";
            $data["parent"]        = ucwords(strtolower($this->PROT_Parent));
            $data["form_act"]      = "/return_status/save";
            /* ----------
             Model
            ----------------------- */
            ...
            $data["fields"][]      = form_text(array("name"=>"description", "label"=>"Description", "mandatory"=>"yes"));
            # ---------------
    ...
    public function save(Request $request) {
        try {
            $response   = $this->qReturnStatus->createData($request);
    ...
    public function edit($id) {
        try {
            $data["title"]        = "Edit Return Code";
            $data["parent"]       = ucwords(strtolower($this->PROT_Parent));
            $data["form_act"]     = "/return_status/update";
            # ---------------
            $id                   = decodedData($id);
            /* ----------
             Model
            ...
            /* ----------
             Source
            ----------------------- */
            $qReturnStatus         = ReturnStatusModel::find($id);
            ...
            $data["fields"][]      = form_text(array("name"=>"name", "label"=>"Name", "mandatory"=>"yes", "readonly"=>"readonly", "value"=>$qReturnStatus->name));
            $data["fields"][]      = form_text(array("name"=>"description", "label"=>"Description", "mandatory"=>"yes", "value"=>$qReturnStatus->description));
            $data["fields"][]      = form_radio(array("name"=>"status", "label"=>"Status", "mandatory"=>"yes", "source"=>$qStatus, "value"=>$qReturnStatus->status));
            # ---------------
    ...
    public function update(Request $request) {
        try {
            $response   = $this->qReturnStatus->updateData($request);
            ...
            $this->logModel->createError($e->getMessage(), "PAGE UPDATE RETURN CODE", "");
            # ---------------
            return view("error.405");
        }
    }
