                        ...
                        <table class="table table-bordered" id="table-document">
                            ...
                            @if(count($items) > 0)
                                    @foreach($items AS $index => $row)
                                        <tr id="tr_item_{{$row->interface_data_detail_id}}">
                                            ...
                                            @else
                                                ...
                                                @endphp
                                            @endif
                                            <td style="text-align: center; {{ $color }}">{{ $row->document_no }}</td>
                                            <td style="{{ $color }}">{{ $row->document_title }}</td>
                                            <td style="text-align: center; {{ $color }}">{{ $row->issue_status_name }}</td>
                                            <td style="text-align: center; {{ $color }}">{{ $row->document_status_name }}</td>
                                            <td style="text-align: center; {{ $color }}">{{ $row->remark }}</td>
                                            ...
                                            @if ($row->status_detail == 1) 
                                                <td style="text-align: center;">
                                                    <a onClick="deleteThis('{{ $row->interface_data_detail_id }}' , '{{$row->interface_data_subfolder_id}}')" title="Hapus" id="btn_delete_{{$row->interface_data_detail_id}}">
...
<script>
    ...
    function deleteThis(detail, id) {
        $.ajax({
            url: "<?=URL::to('/').$delete_url.'/'?>" + detail + '/' + id,
            type: "GET",
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            success: function (response) {
                $("#tr_item_" + detail).remove();
    ...
    $("#button_attach").click(function() {
        ...
        var interface_data_id   = $("#interface_data_id").val();
        var interface_data_subfolder_id   = $("#interface_data_subfolder_id").val();
        var subfolder_name      = $("#subfolder_name").val();
        ...
        formData.append('interface_data_id', interface_data_id);
        formData.append('interface_data_subfolder_id', interface_data_subfolder_id);
        formData.append('subfolder_name', subfolder_name);
                    ...
                    response.data.forEach((element) => {
                        $("#table-document > tbody").append(
                            `<tr>
                                ...
                                <td style="text-align: center;">` + element.remark + `</td>
                                <td style="text-align: center;" title="` + element.interface_data_detail_id + `">
                                    <a href="{{ asset('uploads') }}` + element.document_url + element.document_file + `" target="_blank">
                                        <img src="{{ asset('app/img/icon/eye.png') }}" height="16" class="view-item" style="cursor: pointer">
                                    </a>
                                <td style="text-align: center;" title="` + element.interface_data_detail_id + `"><a onClick="deleteThis(` + element.interface_data_detail_id + `,` + element.interface_data_subfolder_id +` )"><img src="{{ asset('app/img/icon/delete.png') }}" height="16" class="delete-item" style="cursor: pointer"></a></td>
                             </tr>`
                        );
